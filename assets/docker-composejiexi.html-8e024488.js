import{_ as n,o as s,c as a,d as e}from"./app-7aa4a9b1.js";const t={},p=e(`<h1 id="docker-compose解析" tabindex="-1"><a class="header-anchor" href="#docker-compose解析" aria-hidden="true">#</a> Docker-compose解析</h1><h2 id="docker-compose-是什么" tabindex="-1"><a class="header-anchor" href="#docker-compose-是什么" aria-hidden="true">#</a> Docker-compose 是什么？</h2><p>官方定义：Docker Compose 是一个工具，允许通过一个单独的 YAML 文件定义和管理多个 Docker 容器的配置。它简化了多个容器应用的配置过程，允许用户定义整个应用程序的服务、网络、卷等。</p><h2 id="什么场景要用-docker-compose" tabindex="-1"><a class="header-anchor" href="#什么场景要用-docker-compose" aria-hidden="true">#</a> 什么场景要用 docker-compose？</h2><p>一个工具的使用一定有相应的场景，否则它就没有价值。 docker-compose的使用场景是：当我们需要启动多个容器，来完成我们的服务，比如一个后端服务器，一个 redis，一个 mysql，以此类推，而有些容器的启动需要依赖其他容器，这部分可以在 docker-compose 中非常容易的实现，而不需要每次都编写脚本，按照一系列命令来启动容器。</p><p>请参考我目前项目中的一个Docker compose 的配置文件，如下所示：</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">version</span><span class="token punctuation">:</span> <span class="token string">&#39;3.7&#39;</span>
